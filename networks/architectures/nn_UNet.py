...
class ConvBlock(nn.Module):
    def __init__(self, in_channels:int=1, out_channels:int=1, dim:int=2, init_method=init.kaiming_uniform_, dropout=0.05):
...
class ResConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dim:int=2, init_method=init.kaiming_uniform_):
...
class GatedAttentionBlock(nn.Module):
    ...
    """
    def __init__(self, F_g:int, F_l:int, F_int:int, dim:int=2, init_method=init.kaiming_uniform_):
        """
        Args:
...
class UNet(BaseModule):
    def __init__(self, convBlock:'nn.Module'=DoubleConvBlock, deeper_skips:bool=False, num_layers:int=4, base_filters:int=64, in_channels:int=1, out_channels:Union[None,int]=None, convBlock_layer:int=None, filter_function:Literal['constant']='constant', k:float=2., attention:bool=True, dim:int=2, init_method=init.kaiming_uniform_):
        ...
        Args:
            ...
            dim (int): dimension of the network (if inputs are images, 3D tensors...)
        ...
        for i in range(num_layers):
            ...
            if i >= convBlock_layer:
                self.encoders.append(convBlock(in_channels, out_channels, dim=self.dimension, init_method=init_method))
            else:
                self.encoders.append(DoubleConvBlock(in_channels, out_channels, dim=self.dimension, init_method = init_method))
            in_channels = out_channels
        out_channels = filter_sizes[-1]

        # Bottleneck
        self.bottleneck = convBlock(in_channels, out_channels, dim=self.dimension)
        in_channels = out_channels
        ...
        for j in range(self.out_channels):
            ...
            for i in range(num_layers):
                ...
                if self.attention:
                    self.attentions[j].append(GatedAttentionBlock(F_g=out_channels, F_l=out_channels, F_int=out_channels//2, dim=self.dimension, init_method=init_method))
                concat_ch = out_channels * 2
                if i+2 <= num_layers and self.deeperskips:
                    concat_ch += reversed_filters[2+i]
                block = convBlock(concat_ch, out_channels, dim=self.dimension) if num_layers-i >= convBlock_layer else DoubleConvBlock(concat_ch, out_channels, dim=self.dimension, init_method=init_method)
    ...
    def forward(self, x):
        ...
        assert (len(x.shape) > self.dimension+1), LOGGER.error(f"U-Net is defined as {self.dimension}D but input has {len(x.shape)-2} dimensions")
            ...
            for i in range(self.num_layers):
                ...
                if i+2 <= self.num_layers and self.deeperskips:
                    ...
                    if deeper_skip.shape[2:] != xj.shape[2:]:
                        deeper_skip = F.interpolate(deeper_skip, size=xj.shape[2:], mode='trilinear' if self.dimension==3 else 'bilinear', align_corners=False)
