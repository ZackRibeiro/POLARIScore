...
parser.add_argument("--obs_name", required=False, default=None)
parser.add_argument("--obs_suffixes", required=False, nargs="+", default=None)
...
parser.add_argument("--density_correction", required=False, default=True, help="Apply two medius approx to go from the mass-weighted density to core volume density.")
...
if args.obs_name is not None:
    observation = Observation("OrionB","column_density_map")
...
for i,t,m in zip(range(len(trainers)),trainers, args.models):
    ...
    if observation is not None :
        ...
        _, ax = observation.plot_cores_error(ax=global_axes["core_errors"], 
                                             mov_average=0, log_average=50, show_errors=False, correction=args.density_correction, color="black", linestyle=linestyles[i], label=m)
        global_axes["core_errors"] = ax
        ...
        fig_dcmf, _ = observation.plot_dcmf(method="constant", monte_carlo=20, fit=False, bins=15, correction=args.density_correction)
        fig_dcmf.savefig(os.path.join(F_DCMF_PATH,"dcmf_"+m+"."+args.format))

        fig_core_hists, _ = observation.plot_cores_hist(label=m, correction=args.density_correction)
        fig_core_hists.savefig(os.path.join(F_CORE_HISTS,"core_hist_"+m+"."+args.format))

        fig_core_relations, _ = observation.plot_cores_baseline(derived_cores=True, density_correction=args.density_correction, invert_xy=True, x_coldens=True, mov_average=1, fit=True, forced_label=m)
        fig_core_relations.savefig(os.path.join(F_CORE_RELATIONS,"core_relations_"+m+"."+args.format))
